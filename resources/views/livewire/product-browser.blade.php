<div class="mx-auto sm:px-6 lg:px-8 max-w-7xl py-12 grid grid-cols-6 gap-4">
            ...
            <div class="space-y-6">
                @if ($category->products->count())
                    <div class="space-y-1">
                        ...
                        <div class="flex items-center space-x-2">
                            <input type="range" min="100" max="{{ $maxPrice }}" wire:model="priceRange.max">
                        </div>
                    </div>
                @endif
                ...
                @if ($products->count())
                    @foreach ($filters as $name => $filter)
                        <div class="space-y-1">
                            <div class="font-semibold">{{ \Illuminate\Support\Str::title($name) }}</div>
                            @foreach ($filter as $option => $count)
                                <div class="flex items-center space-x-2">
                                    <input wire:model="queryFilters.{{ $name }}"
                                           type="checkbox" id="{{ $name }}_{{ strtolower($option) }}" value="{{ $option }}">
                                    <label for="{{ $name }}_{{ strtolower($option) }}">{{ $option }} ({{ $count }})</label>
                                </div>
                            @endforeach
                        </div>
                    @endforeach
    ...
    <div class="col-span-5 sm:px-6 lg:px-8">
        <div class="mb-6">
            Found {{ $products->count() }} {{ \Illuminate\Support\Str::plural('product', $products->count()) }} matching your filters
        </div>
